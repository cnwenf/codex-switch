...
export const PROVIDER_PRESETS = freeze([
  ...
  preset({ id: 'bailian', name: '阿里云百炼', group: GROUPS.direct, compatibility: 'limited', tokenEnv: 'DASHSCOPE_API_KEY', options: [{ name: 'region', type: 'select', default: 'cn-beijing', choices: ['cn-beijing', 'ap-southeast-1', 'us-west-1'] }, { name: 'workspace_id', type: 'text', default: '' }], buildBaseUrl: bailianBaseUrl }),
...
const HOST_TYPES = Object.freeze([
  ...
  ['bedrock-mantle.', 'aws-bedrock'], ['api.cloudflare.com', 'cloudflare-workers-ai'],
...
function normalizeOptions(preset, input) {
  ...
  if (preset.id === 'bailian') {
    if (!preset.options[0].choices.includes(options.region)) throw new Error('Invalid Bailian region');
    assertIdentifier(options.workspace_id, 'workspace ID', true);
  }
...
function bailianBaseUrl({ region, workspace_id: workspaceId }) {
  const host = { 'cn-beijing': 'dashscope.aliyuncs.com', 'ap-southeast-1': 'dashscope-intl.aliyuncs.com', 'us-west-1': 'dashscope-us.aliyuncs.com' }[region];
  return workspaceId
    ? `https://${host}/api/v1/apps/${encodeURIComponent(workspaceId)}/compatible-mode/v1`
...
function normalizeAzureEndpoint(value) {
  ...
  const pathname = url.pathname.replace(/\/openai\/v1\/?$/, '');
